defmodule Combo.Endpoint do
  ...
  `{MyApp.Web.Endpoint, opts}`.
    ...
    * `:cache_static_manifest` - a path to a JSON manifest file that contains
      ...
      "priv/static/cache_manifest.json" which is the file generated by
      `mix combo.digest`. It can be either a string containing a file system
      path, or a tuple containing the application name and the path within that
      application.

    * `:cache_static_manifest_latest` - a map of the static files pointing to
      their digest version. This is automatically loaded from the file
      specified by the `:cache_static_manifest` option on boot. However, if you
  ...
  Checks if Endpoint's web server has been configured to start.
    ...
    * `otp_app` - The OTP app running the endpoint, such as `:my_app`.
    * `endpoint` - The endpoint module, such as `MyAppWeb.Endpoint`.
