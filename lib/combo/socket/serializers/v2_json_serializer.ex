defmodule Combo.Socket.V2.JSONSerializer do
  ...
  def fastlane!(%Broadcast{payload: %{}} = msg) do
    data = Combo.json_module().encode_to_iodata!([nil, nil, msg.topic, msg.event, msg.payload])
  ...
  def encode!(%Reply{} = reply) do
    ...
    {:socket_push, :text, Combo.json_module().encode_to_iodata!(data)}
  end
  ...
  defp decode_text(raw_message) do
    [join_ref, ref, topic, event, payload | _] = Combo.json_module().decode!(raw_message)
