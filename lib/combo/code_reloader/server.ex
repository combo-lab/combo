defmodule Combo.CodeReloader.Server do
  ...
  def init(:ok) do
    ...
    # compilation as that's effectively checked on boot. On boot, Elixir checks
    # if all dependencies match the lock. If they don't, Elixir refuses to boot,
    # and forces a "mix deps.get", which then writes a .mix/compile.lock that the
    # compiler checks.
    #
    # Meanwhile, Combo has to answer the question: has the lock file changed?
    ...
    # not necessarily force Elixir to compile. As established above, the mix.lock
  ...
  # Deployments may boot with Mix loaded but never started, in which case
  # Mix.Project.config/0 and friends exit, as they rely on the Mix.ProjectStack
  # process.
  ...
  defp can_symlink?() do
    ...
    symlink = Path.join(Path.dirname(build_path), "__combo__")
