defmodule Combo.Presence do
  ...
  By default, `Combo.Channel.Server` is used, which is the same dispatcher
  used by channels. To customize the dispatcher, pass the `:dispatcher` option
  when using `Combo.Presence`:
  ...
  See `m:Combo.PubSub#module-custom-dispatching` for more information on
  custom dispatchers.
  ...
  defmodule Tracker do
    ...
    def start_link({module, task_supervisor, opts}) do
      ...
      dispatcher = opts[:dispatcher] || Combo.Channel.Server
  ...
  def handle_info({task_ref, {:combo, ref, computed_diffs}}, state) do
    ...
    Enum.each(computed_diffs, fn {topic, presence_diff} ->
      broadcast = %Combo.Socket.Broadcast{
        topic: topic,
        event: "presence_diff",
        payload: presence_diff
      }

      Combo.PubSub.local_broadcast(state.pubsub_server, topic, broadcast, state.dispatcher)
    end)
