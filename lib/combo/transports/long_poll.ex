defmodule Combo.Transports.LongPoll do
  ...
  ## Options
    ...
    * `:window_ms` - how long the client can wait for new messages in its poll
      request in milliseconds (ms).
      Defaults to `10_000`.

    * `:pubsub_timeout_ms` - how long a request can wait for the pubsub layer
      to respond in milliseconds (ms).
      Defaults to `2000`.

    * `:crypto` - options for verifying and signing the token, accepted by
      `Combo.Token`.
  ...
  defp send_json(conn, data) do
    ...
    |> send_resp(200, Combo.json_library().encode_to_iodata!(data))
  end
end
