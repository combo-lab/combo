defmodule Combo.LoggerTest do
  ...
  describe "filter_values/2 with discard strategy" do
    test "in top level map" do
      ...
      assert Combo.Logger.filter_values(values, ["password"]) ==
               %{"foo" => "bar", "password" => "[FILTERED]"}

      assert Combo.Logger.filter_values(values, Combo.Logger.compile_filter(["password"])) ==
               %{"foo" => "bar", "password" => "[FILTERED]"}
    end
  ...
  describe "filter_values/2 with keep strategy" do
    test "discards values not specified in params" do
      ...
      assert Combo.Logger.filter_values(values, {:keep, []}) ==
               %{"foo" => "[FILTERED]", "password" => "[FILTERED]", "file" => "[FILTERED]"}

      assert Combo.Logger.filter_values(values, Combo.Logger.compile_filter({:keep, []})) ==
               %{"foo" => "[FILTERED]", "password" => "[FILTERED]", "file" => "[FILTERED]"}
    end
    ...
    test "keeps values that are specified in params" do
      ...
      assert Combo.Logger.filter_values(values, {:keep, ["foo", "file"]}) ==
    ...
    test "keeps all values under keys that are kept" do
      ...
      assert Combo.Logger.filter_values(values, {:keep, ["foo"]}) ==
               %{"foo" => %{"bar" => 1, "baz" => 2}}
    end
