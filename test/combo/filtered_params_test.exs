defmodule Combo.FilteredParamsTest do
  use ExUnit.Case, async: true

  alias Combo.FilteredParams

  describe "discard strategy" do
    test "in top level map" do
      values = %{"foo" => "bar", "password" => "should_not_show"}

      assert FilteredParams.filter(values, {:discard, ["password"]}) ==
  ...
  describe "keep strategy" do
    ...
    test "keeps values that are specified in params" do
      ...
      assert FilteredParams.filter(values, {:keep, ["foo", "file"]}) ==
    ...
    test "keeps all values under keys that are kept" do
      ...
      assert FilteredParams.filter(values, {:keep, ["foo"]}) ==
               %{"foo" => %{"bar" => 1, "baz" => 2}}
    end
