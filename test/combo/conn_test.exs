...
defmodule Combo.Conn.Test do
  ...
  describe "root_layout/_" do
    ...
    test "with setting root layout" do
      conn = conn(:get, "/") |> put_root_layout(html: {DemoLayout, :app})

      assert root_layout(conn) == false
      assert root_layout(conn, nil) == false

      assert root_layout(conn, :html) == {DemoLayout, :app}
      assert root_layout(conn, "html") == {DemoLayout, :app}
      ...
      conn =
        conn(:get, "/")
        |> put_root_layout(html: {DemoLayout, :app})
        |> put_format("html")

      assert root_layout(conn) == {DemoLayout, :app}
      assert root_layout(conn, nil) == {DemoLayout, :app}
  ...
  test "put_root_layout/2" do
    conn = conn(:get, "/")

    conn = put_root_layout(conn, html: {DemoLayout, :app})
    assert root_layout(conn, :html) == {DemoLayout, :app}

    conn = put_root_layout(conn, html: {DemoLayout, :app}, print: {DemoLayout, :print})
    conn = put_format(conn, :html)
    assert root_layout(conn) == {DemoLayout, :app}
    conn = put_format(conn, :print)
    assert root_layout(conn) == {DemoLayout, :print}
    ...
    assert_raise Plug.Conn.AlreadySentError,
                 ...
                 the response was already sent.
                     ...
                     Request path: /
                     Method: GET
                     Root layout formats: [html: {DemoLayout, :app}]
                 """,
                 fn ->
                   put_root_layout(sent_conn(), html: {DemoLayout, :app})
                 end
  end
