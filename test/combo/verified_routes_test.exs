modules = [
  Combo.VerifiedRoutesTest.PostController,
  Combo.VerifiedRoutesTest.UserController
]
...
defmodule Combo.VerifiedRoutesTest do
  ...
  defp conn_with_endpoint(endpoint \\ Endpoint) do
    conn(:get, "/") |> Plug.Conn.put_private(:combo_endpoint, endpoint)
  end
  ...
  def conn_with_script_name(script_name \\ ~w(api)) do
    conn = Plug.Conn.put_private(conn(:get, "/"), :combo_endpoint, ScriptName)
  ...
  use Combo.VerifiedRoutes, statics: ~w(images), endpoint: Endpoint, router: Router
  ...
  describe "with script name" do
    ...
    test "combo_router_url with URI takes precedence over endpoint" do
      ...
      assert url(conn, ~p"/admin/new/messages/1") ==
               "https://example.com:123/path/admin/new/messages/1"
    end
  ...
  describe "warnings" do
    ...
    test "~p warns on unmatched path" do
      ...
      assert warnings =~
               ~s|no route path for Combo.VerifiedRoutesTest.Router matches "/unknown"|

      assert warnings =~
               ~s|no route path for Combo.VerifiedRoutesTest.Router matches "/unknown/123"|
    ...
    test "~p warns on warn_on_verify: true route" do
      ...
      assert warnings =~
               ~s|no route path for Combo.VerifiedRoutesTest.Router matches "/should-warn/foobar"|
    after
