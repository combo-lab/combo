...
describe("LongPoll", () => {
  ...
  it("should send the session token in a header", () => {
    ...
    Ajax.request.mockImplementationOnce(
    ...
    expect(Ajax.request).toHaveBeenLastCalledWith(
      ...
      { "Accept": "application/json", "X-Combo-Longpoll-Token": "token123" },
      null,
      expect.any(Number),
      expect.any(Function),
      expect.any(Function),
  ...
  it("should send the token header on batched pushes", () => {
    ...
    expect(Ajax.request).toHaveBeenLastCalledWith(
      "POST",
      "http://localhost/socket/longpoll",
      { "Content-Type": "application/x-ndjson", "X-Combo-Longpoll-Token": "token123" },
      "msg1",
      expect.any(Number),
      expect.any(Function),
      expect.any(Function),
