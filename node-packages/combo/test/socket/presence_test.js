import { Presence } from '../../src/socket'
...
const fixtures = {
  joins() {
    return { u1: { metas: [{ id: 1, combo_ref: '1.2' }] } }
  },
  leaves() {
    return { u2: { metas: [{ id: 2, combo_ref: '2' }] } }
  },
  state() {
    return {
      u1: { metas: [{ id: 1, combo_ref: '1' }] },
      u2: { metas: [{ id: 2, combo_ref: '2' }] },
      u3: { metas: [{ id: 3, combo_ref: '3' }] },
    }
  },
}
...
describe('syncState', () => {
  it('syncs empty state', () => {
    let newState = { u1: { metas: [{ id: 1, combo_ref: '1' }] } }
    let state = {}
  ...
  it('onJoins new presences and onLeave\'s left presences', () => {
    ...
    let state = { u4: { metas: [{ id: 4, combo_ref: '4' }] } }
    let joined = {}
    let left = {}
    ...
    expect(joined).toEqual({
      u1: { current: undefined, newPres: { metas: [{ id: 1, combo_ref: '1' }] } },
      u2: { current: undefined, newPres: { metas: [{ id: 2, combo_ref: '2' }] } },
      u3: { current: undefined, newPres: { metas: [{ id: 3, combo_ref: '3' }] } },
    })
    expect(left).toEqual({
      u4: { current: { metas: [] }, leftPres: { metas: [{ id: 4, combo_ref: '4' }] } },
    })
  })
  ...
  it('onJoins only newly added metas', () => {
    let newState = {
      u3: {
        metas: [
          { id: 3, combo_ref: '3' },
          { id: 3, combo_ref: '3.new' },
        ],
      },
    }
    let state = { u3: { metas: [{ id: 3, combo_ref: '3' }] } }
    let joined = []
    let left = []
    const onJoin = (key, current, newPres) => {
    ...
    expect(joined).toEqual([
      [
        'u3',
        {
          current: { metas: [{ id: 3, combo_ref: '3' }] },
          newPres: { metas: [{ id: 3, combo_ref: '3.new' }] },
        },
      ],
    ])
    expect(left).toEqual([])
...
describe('syncDiff', () => {
  it('syncs empty state', () => {
    let joins = { u1: { metas: [{ id: 1, combo_ref: '1' }] } }
  ...
  it('removes meta while leaving key if other metas exist', () => {
    ...
    state = Presence.syncDiff(state, {
      joins: {},
      leaves: { u1: { metas: [{ id: 1, combo_ref: '1' }] } },
    })

    expect(state).toEqual({
      u1: { metas: [{ id: 1, combo_ref: '1.2' }] },
    })
  })
})
...
describe('list', () => {
  it('lists full presence by default', () => {
    ...
    expect(Presence.list(state)).toEqual([
      { metas: [{ id: 1, combo_ref: '1' }] },
      { metas: [{ id: 2, combo_ref: '2' }] },
      { metas: [{ id: 3, combo_ref: '3' }] },
    ])
  })
  ...
  it('lists with custom function', () => {
    let state = {
      u1: {
        metas: [
          { id: 1, combo_ref: '1.first' },
          { id: 1, combo_ref: '1.second' },
        ],
      },
    }
    ...
    expect(Presence.list(state, listBy)).toEqual([{ id: 1, combo_ref: '1.first' }])
  })
})
...
describe('instance', () => {
  it('syncs state and diffs', () => {
    ...
    let user1 = { metas: [{ id: 1, combo_ref: '1' }] }
    let user2 = { metas: [{ id: 2, combo_ref: '2' }] }
    ...
    expect(presence.list(listByFirst)).toEqual([{ id: 2, combo_ref: '2' }])
  })
  ...
  it('applies pending diff if state is not yet synced', () => {
    ...
    let user1 = { metas: [{ id: 1, combo_ref: '1' }] }
    let user2 = { metas: [{ id: 2, combo_ref: '2' }] }
    let user3 = { metas: [{ id: 3, combo_ref: '3' }] }
    let newState = { u1: user1, u2: user2 }
    let leaves = { u2: user2 }
    ...
    expect(onLeaves).toEqual([
      { id: 'u2', current: { metas: [] }, leftPres: { metas: [{ id: 2, combo_ref: '2' }] } },
    ])

    expect(presence.list(listByFirst)).toEqual([{ id: 1, combo_ref: '1' }])
    expect(presence.pendingDiffs).toEqual([])
    expect(onJoins).toEqual([
      { id: 'u1', current: undefined, newPres: { metas: [{ id: 1, combo_ref: '1' }] } },
      { id: 'u2', current: undefined, newPres: { metas: [{ id: 2, combo_ref: '2' }] } },
    ])
    ...
    expect(presence.list(listByFirst)).toEqual([{ id: 3, combo_ref: '3' }])
  })
  ...
  it('updates existing meta for a presence update (leave + join)', () => {
    ...
    let user1 = { metas: [{ id: 1, combo_ref: '1' }] }
    let user2 = { metas: [{ id: 2, name: 'chris', combo_ref: '2' }] }
    let newState = { u1: user1, u2: user2 }
    ...
    expect(presence.list((id, { metas: metas }) => metas)).toEqual([
      [{ id: 1, combo_ref: '1' }],
      [{ id: 2, name: 'chris', combo_ref: '2' }],
    ])

    let leaves = { u2: user2 }
    let joins = { u2: { metas: [{ id: 2, name: 'chris.2', combo_ref: '2.2', combo_ref_prev: '2' }] } }
    channelStub.trigger('presence_diff', { joins, leaves })

    expect(presence.list((id, { metas: metas }) => metas)).toEqual([
      [{ id: 1, combo_ref: '1' }],
      [{ id: 2, name: 'chris.2', combo_ref: '2.2', combo_ref_prev: '2' }],
    ])

    expect(onJoins).toEqual([
      {
        id: 'u2',
        current: { metas: [{ id: 2, name: 'chris', combo_ref: '2' }] },
        newPres: { metas: [{ id: 2, name: 'chris.2', combo_ref: '2.2', combo_ref_prev: '2' }] },
      },
    ])
  })
})
