...
describe("binary", () => {
  ...
  it("encodes non-ASCII metadata as UTF-8", (done) => {
    ...
    Serializer.encode(
      { join_ref: joinRef, ref: ref, topic: topic, event: event, payload: buffer },
      (result) => {
        ...
        let offset = 5
        expect(Array.from(bytes.slice(offset, offset + joinRefBytes.byteLength))).toEqual(
          Array.from(joinRefBytes),
        )
        offset += joinRefBytes.byteLength
        expect(Array.from(bytes.slice(offset, offset + refBytes.byteLength))).toEqual(
          Array.from(refBytes),
        )
        offset += refBytes.byteLength
        expect(Array.from(bytes.slice(offset, offset + topicBytes.byteLength))).toEqual(
          Array.from(topicBytes),
        )
        offset += topicBytes.byteLength
        expect(Array.from(bytes.slice(offset, offset + eventBytes.byteLength))).toEqual(
          Array.from(eventBytes),
        )
  ...
  it("throws when a metadata field exceeds 255 UTF-8 bytes", () => {
    ...
    expect(() => {
      Serializer.encode(
        { join_ref: "0", ref: "1", topic: big, event: "e", payload: buffer },
        () => {},
      )
    }).toThrow(/topic/)
  })
