 ...
 * @param {(string|function)} [opts.authToken] - the optional authentication token to be
 * exposed on the server under the `:auth_token` connect_info key. If it is a function,
 * the function's return value must be a string.
...
export default class Socket {
  constructor(endPoint, opts = {}) {
    ...
    comboWindow.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "hidden") {
        this.pageHidden = true
      } else {
        this.pageHidden = false
        // reconnect immediately if the connection was not closed gracefully
